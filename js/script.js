...
function newGame() {
  ...
  displayTaskImgs();
  messageEl.textContent =
    "Odaberi sliku sa svake strane koja se odnosi na zadano godišnje doba.";
}
...
function clickImg(e) {
  ...
  if (imgEl.dataset.id === "wc" || imgEl.dataset.id === "tp") {
...
function checkAnswer(bollArr) {
  ...
  if (bool1 && bool2) {
    if (answerCircle === task && answerSquare === task) {
      messageEl.textContent =
        "Odlično! Pokreni igru ponovno i nastavi vježbati.";
...
function messageWrongAnswers() {
  if (level === "beginner") {
    ...
    switch (true) {
      ...
      case answerCircle !== task:
        messageEl.textContent =
          "Jedan odgovor je netočan. Odaberi neku drugu sliku ispod naslova Dijelovi stabla.";
        break;
      case answerSquare !== task: {
        messageEl.textContent =
